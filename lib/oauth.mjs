 ...
 * @returns {Promise<{type: "success", refresh: string, access: string, expires: number, email?: string, accountUuid?: string} | {type: "failed", error?: string}>}
 */
export async function exchange(code, verifier) {
  try {
    ...
    return {
      ...
      accountUuid: typeof json.account?.uuid === "string" ? json.account.uuid : undefined,
    };
  } catch (error) {
    return {
      type: "failed",
...
/**
 * Fetch OAuth profile information for an access token.
 * Mirrors Claude Code's `/api/oauth/profile` lookup for account identity.
 *
 * @param {string} accessToken
 * @param {{ signal?: AbortSignal }} [options]
 * @returns {Promise<{accountUuid?: string, email?: string, organizationUuid?: string} | null>}
 */
export async function fetchOAuthProfile(accessToken, options = {}) {
  try {
    ...
    const profile = {
      ...(typeof json?.account?.uuid === "string" ? { accountUuid: json.account.uuid } : {}),
      ...(typeof json?.account?.email === "string" ? { email: json.account.email } : {}),
      ...(typeof json?.organization?.uuid === "string" ? { organizationUuid: json.organization.uuid } : {}),
    };

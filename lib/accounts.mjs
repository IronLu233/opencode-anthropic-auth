import { loadAccounts, saveAccounts, createDefaultStats, hasAccountsStorageFile } from "./storage.mjs";
...
/**
 * In-memory account pool for selection, health tracking, and stats merging.
 *
 * Runtime mutations flow through disk storage (`loadAccounts`/`saveAndSync`) in
 * the CLI and plugin entrypoints. This class is responsible for runtime
 * selection, token bucket / health bookkeeping, and merge-on-save behavior.
 */
export class AccountManager {
   ...
   * @param {AnthropicAuthConfig} config
   * @param {{refresh: string, access?: string, expires?: number, accountUuid?: string} | null} [authFallback]
   * @returns {Promise<AccountManager>}
   */
  static async load(config, authFallback) {
    const manager = new AccountManager(config);
    const storageExists = hasAccountsStorageFile();
    ...
    if (stored) {
      manager.#accounts = stored.accounts.map((acc, index) => ({
        ...
        access: acc.access,
        expires: acc.expires,
        tokenUpdatedAt: acc.tokenUpdatedAt,
        addedAt: acc.addedAt,
      ...
      if (authFallback && manager.#accounts.length > 0) {
        ...
        if (match) {
          ...
          if (shouldAdoptFallback) {
            ...
            match.tokenUpdatedAt = Math.max(match.tokenUpdatedAt || 0, Date.now());
          }
          if (typeof authFallback.accountUuid === "string" && !match.accountUuid) {
            match.accountUuid = authFallback.accountUuid;
          }
        }
      }
      ...
      manager.#accounts = [
        {
          ...
          index: 0,
          email: undefined,
          accountUuid: authFallback.accountUuid,
   ...
   * each other's stats.
   * @param {{ preserveDiskState?: boolean }} [options]
   * @returns {Promise<void>}
   */
  async saveToDisk(options = {}) {
    const preserveDiskState = options.preserveDiskState !== false;
    // Read current disk state to merge stats
    /** @type {AccountStorage | null} */
    let diskData = null;
    /** @type {((account: { id?: string, addedAt?: number, refreshToken?: string }) => import('./storage.mjs').AccountMetadata | null) | null} */
    let findDiskAccount = null;
    try {
      diskData = await loadAccounts();
      if (diskData) {
        findDiskAccount = createDiskAccountMatcher(diskData.accounts);
      }
    } catch {
    ...
    const storage = {
      ...
      accounts: this.#accounts
        .map((acc) => {
          ...
          const diskAcc = findDiskAccount?.(acc) ?? null;

          if (preserveDiskState && diskData && !diskAcc) {
            return null;
          }
          ...
          const diskTokenUpdatedAt = diskAcc?.tokenUpdatedAt || 0;
          const freshestAuth =
            diskAcc && diskTokenUpdatedAt > memTokenUpdatedAt
              ? {
                  accountUuid: diskAcc.accountUuid,
              ...
              : {
                  accountUuid: acc.accountUuid || diskAcc?.accountUuid,
                  refreshToken: acc.refreshToken,
                  access: acc.access,
                  expires: acc.expires,
          ...
          acc.accountUuid = freshestAuth.accountUuid;

          return {
            id: acc.id,
            email: acc.email,
            accountUuid: freshestAuth.accountUuid,
            refreshToken: freshestAuth.refreshToken,
            access: freshestAuth.access,
            expires: freshestAuth.expires,
            tokenUpdatedAt: freshestAuth.tokenUpdatedAt,
            addedAt: acc.addedAt,
            lastUsed: acc.lastUsed,
            enabled: preserveDiskState && diskAcc ? diskAcc.enabled : acc.enabled,
      ...
      activeIndex: preserveDiskState && diskData ? diskData.activeIndex : Math.max(0, this.#currentIndex),
    };
  ...
  async syncActiveIndexFromDisk() {
    ...
    const memSnapshot = this.#accounts
      .map((acc) => `${acc.id}:${acc.refreshToken}:${acc.accountUuid || ""}:${acc.enabled ? 1 : 0}`)
      .join("|");
    ...
    const diskSnapshot = stored.accounts
      .map((acc) => {
        ...
        return `${resolvedId}:${acc.refreshToken}:${acc.accountUuid || ""}:${acc.enabled ? 1 : 0}`;
      })
      .join("|");
    ...
    if (diskSnapshot !== memSnapshot) {
      ...
      this.#accounts = stored.accounts.map((acc, index) => {
        ...
        return {
          ...
          accountUuid: acc.accountUuid ?? existing?.accountUuid,
          refreshToken: acc.refreshToken,
          access: acc.access ?? existing?.access,
          expires: acc.expires ?? existing?.expires,
          tokenUpdatedAt: acc.tokenUpdatedAt ?? existing?.tokenUpdatedAt ?? acc.addedAt,
          addedAt: acc.addedAt,
          lastUsed: acc.lastUsed,
          enabled: acc.enabled,
          rateLimitResetTimes: acc.rateLimitResetTimes,

...
export class AccountManager {
  ...
  addAccount(refreshToken, accessToken, expires, email) {
    ...
    if (existing) {
      applyAuthenticatedAccount(existing, {
        refresh: refreshToken,
        access: accessToken,
        expires,
        ...(email ? { email } : {}),
      });
      // applyAuthenticatedAccount → applyOAuthCredentials always sets snake_case
      // token_updated_at (disk format); ManagedAccount uses camelCase tokenUpdatedAt.
      // Fallback is defensive only — token_updated_at is always set above.
      existing.tokenUpdatedAt = existing.token_updated_at ?? Date.now();
      delete existing.token_updated_at;
      return existing;
    }

    const now = Date.now();
    const base = createOAuthAccountRecord(
      { refresh: refreshToken, access: accessToken, expires, ...(email ? { email } : {}) },
      now,
    );
    /** @type {ManagedAccount} */
    const account = {
      ...
      tokenUpdatedAt: base.token_updated_at,
      addedAt: base.addedAt,
      lastUsed: base.lastUsed,
      enabled: base.enabled,
      rateLimitResetTimes: base.rateLimitResetTimes,
      consecutiveFailures: base.consecutiveFailures,
      lastFailureTime: base.lastFailureTime,
      lastSwitchReason: "initial",
      stats: base.stats ?? createDefaultStats(now),
    };

...
describe("request headers profile", () => {
  ...
  it("maps claude-cli-default alias to current pinned profile", () => {
    expect(getHeaderProfile("claude-cli-default")).toEqual(getHeaderProfile("claude-cli-2.1.80"));
  });
...
describe("getBillingHeaderBlock", () => {
  it("returns correct format matching official Claude Code pattern", () => {
    ...
    expect(block).toMatch(
      /^x-anthropic-billing-header: cc_version=2\.1\.80\.[0-9a-f]{3}; cc_entrypoint=cli; cch=00000;$/,
    );
  });

  it("includes cc_version from the profile", () => {
    const block = getBillingHeaderBlock("claude-cli-2.1.80");
    expect(block).toContain("cc_version=2.1.80.");
  });
...
it("keeps the 2.1.80 profile pinned as the default alias target", () => {

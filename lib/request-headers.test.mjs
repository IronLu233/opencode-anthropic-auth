...
describe("request headers profile", () => {
  ...
  it("maps claude-cli-default alias to current pinned profile", () => {
    expect(getHeaderProfile("claude-cli-default")).toEqual(getHeaderProfile("claude-cli-2.1.97"));
  });
  ...
  it("includes context-management beta in base betas for all models", () => {
    const betas = getDefaultBetas(DEFAULT_HEADER_PROFILE, "claude-opus-4-1");
    expect(betas).toContain("context-management-2025-06-27");
  });

  it("includes context-management beta for sonnet models too", () => {
  ...
  it("current profiles contain expected header keys", () => {
    for (const profileName of ["claude-cli-2.1.90", "claude-cli-2.1.92", "claude-cli-2.1.97"]) {
  ...
  it("getDefaultBetas with null model returns base betas including context-management", () => {
    const betas = getDefaultBetas(DEFAULT_HEADER_PROFILE, null);
    expect(betas).toContain("context-management-2025-06-27");
    expect(betas.length).toBeGreaterThan(0);
  });

  it("getDefaultBetas with undefined model returns base betas including context-management", () => {
    const betas = getDefaultBetas(DEFAULT_HEADER_PROFILE, undefined);
    expect(betas).toContain("context-management-2025-06-27");
    expect(betas.length).toBeGreaterThan(0);
  });

  it("getDefaultBetas with dated sonnet model includes context-management in base", () => {
    const betas = getDefaultBetas(DEFAULT_HEADER_PROFILE, "claude-sonnet-4-5-20250514");
    expect(betas).toContain("context-management-2025-06-27");
  });

  it("getDefaultBetas with dated opus model includes context-management in base", () => {
  ...
  it("HEADER_PROFILES contains the expected profile keys", () => {
    ...
    expect(HEADER_PROFILES).toHaveProperty("claude-cli-2.1.97");
  ...
  it("beta base list contains expected entries for current default", () => {
    ...
    expect(base).toContain("context-management-2025-06-27");
  ...
  it("keeps the 2.1.92 profile pinned for compatibility", () => {
  ...
  it("keeps the 2.1.97 profile pinned as the default alias target", () => {
    const profile = getHeaderProfile("claude-cli-2.1.97");
    const priorLatest = getHeaderProfile("claude-cli-2.1.92");
    expect(profile.headers["user-agent"]).toBe("claude-cli/2.1.97 (external, cli)");
    expect(profile.ccVersion).toBe("2.1.97");
    expect(profile.betaBase).toEqual(getHeaderProfile("claude-cli-2.1.90").betaBase);
    expect(profile.betaByModel).toEqual(priorLatest.betaByModel);
    expect({ ...profile.headers, "user-agent": priorLatest.headers["user-agent"] }).toEqual(priorLatest.headers);
  });
});
...
describe("getBillingHeaderBlock", () => {
  it("returns correct format matching official Claude Code pattern", () => {
    ...
    expect(block).toMatch(
      /^x-anthropic-billing-header: cc_version=2\.1\.97\.[0-9a-f]{3}; cc_entrypoint=cli; cch=00000;$/,
    );
  });

  it("includes cc_version from the profile", () => {
    const block = getBillingHeaderBlock("claude-cli-2.1.97");
    expect(block).toContain("cc_version=2.1.97.");
  });
...
describe("billing fingerprint helpers", () => {
  it("uses empty string when there is no user message", () => {
    ...
    expect(computeBillingFingerprint("", "2.1.97")).toBe("d5e");
  });
  ...
  it("uses string content from the first user message", () => {
    ...
    expect(computeBillingFingerprint(extractFirstUserMessageText(messages), "2.1.97")).toBe("172");
  });
  ...
  it("uses empty string when array user content has no text block", () => {
    ...
    expect(computeBillingFingerprint(extractFirstUserMessageText(messages), "2.1.97")).toBe("d5e");
  });

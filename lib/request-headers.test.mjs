...
describe("request headers profile", () => {
  ...
  it("maps claude-cli-default alias to current pinned profile", () => {
    expect(getHeaderProfile("claude-cli-default")).toEqual(getHeaderProfile("claude-cli-2.1.90"));
  });
  ...
  it("HEADER_PROFILES contains the expected profile keys", () => {
    ...
    expect(HEADER_PROFILES).toHaveProperty("claude-cli-2.1.80");
  ...
  it("beta base list contains expected entries for current default", () => {
    ...
    expect(base).toContain("task-budgets-2026-03-13");
...
describe("getBillingHeaderBlock", () => {
  it("returns correct format matching official Claude Code pattern", () => {
    ...
    expect(block).toMatch(
      /^x-anthropic-billing-header: cc_version=2\.1\.90\.[0-9a-f]{3}; cc_entrypoint=cli; cch=00000;$/,
    );
  });

  it("includes cc_version from the profile", () => {
    const block = getBillingHeaderBlock("claude-cli-2.1.90");
    expect(block).toContain("cc_version=2.1.90.");
  });
...
describe("billing fingerprint helpers", () => {
  it("uses empty string when there is no user message", () => {
    ...
    expect(computeBillingFingerprint("", "2.1.80")).toBe("7aa");
  });
  ...
  it("uses string content from the first user message", () => {
    ...
    expect(computeBillingFingerprint(extractFirstUserMessageText(messages), "2.1.80")).toBe("076");
  });
  ...
  it("uses empty string when array user content has no text block", () => {
    ...
    expect(computeBillingFingerprint(extractFirstUserMessageText(messages), "2.1.80")).toBe("7aa");
  });
...
it("keeps the 2.1.80 profile pinned for backward compat", () => {
  const profile = getHeaderProfile("claude-cli-2.1.80");
  expect(profile.headers["user-agent"]).toBe("claude-cli/2.1.80 (external, cli)");
  expect(profile.betaBase).toContain("advanced-tool-use-2025-11-20");
});

it("keeps the 2.1.90 profile pinned as the default alias target", () => {
  ...
  expect(profile.betaBase).toContain("task-budgets-2026-03-13");
  expect(profile.betaBase).toContain("advanced-tool-use-2025-11-20");
  expect(profile.betaBase).not.toContain("adaptive-thinking-2026-01-28");
});

 ...
 * @property {boolean} billing_header - Include billing header system block (busts prompt cache; default true)
 * @property {boolean} cch_signing - Replace the billing header cch placeholder after serialization (default true)
 */
...
function validateConfig(raw) {
  ...
  if (raw.headers && typeof raw.headers === "object") {
    ...
    if (typeof h.emulation_profile === "string" && h.emulation_profile.trim()) {
      const profile = h.emulation_profile.trim();
      if (HEADER_PROFILES[profile]) {
        config.headers.emulation_profile = profile;
      }
    }
    ...
    if (typeof h.cch_signing === "boolean") {
      config.headers.cch_signing = h.cch_signing;
    }
  }

...
describe("loadAccounts", () => {
  ...
  it("loads valid accounts", async () => {
    ...
    expect(result.accounts[0].accountUuid).toBe("account-uuid-123");
...
describe("saveAccounts", () => {
  ...
  it("merges auth fields from fresher disk state", async () => {
    ...
    expect(written.accounts[0].tokenUpdatedAt).toBe(2000);
  });
  ...
  it("matches id-less disk accounts by addedAt during freshness merge", async () => {
    ...
    expect(written.accounts[0].tokenUpdatedAt).toBe(3000);
  });
  ...
  it("round-trips accountUuid fields", async () => {
    const storage = {
      version: 1,
      accounts: [
        {
    ...
    const written = JSON.parse(fs.writeFile.mock.calls[0][1]);
    expect(written.accounts[0].accountUuid).toBe("account-uuid-123");
  });
});

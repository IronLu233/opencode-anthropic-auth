...
/**
 * Replacement prompt derived from upstream OpenCode's Anthropic prompt, with
 * the build-mode reminder block appended so the whole OpenCode prompt segment
 * can be swapped in one pass.
 *
 * Keep this text free of generic OpenCode/opencode mentions: the fetch-layer
 * body transform still performs a later compatibility rewrite on system text.
 */
...
export function replaceBoundedAnthropicSystemPrompt(systemPrompt) {
  ...
  for (let i = 0; i < matches.length; i++) {
    ...
    if (nextMatch && nextMatch.index < end.index) continue;

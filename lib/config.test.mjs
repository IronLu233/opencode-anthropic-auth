...
describe("DEFAULT_CONFIG", () => {
  ...
  it("has header emulation defaults", () => {
    expect(DEFAULT_CONFIG.headers.emulation_profile).toBe("claude-cli-2.1.97");
    expect(DEFAULT_CONFIG.headers.overrides).toEqual({});
    expect(DEFAULT_CONFIG.headers.disable).toEqual([]);
    expect(DEFAULT_CONFIG.headers.billing_header).toBe(true);
    expect(DEFAULT_CONFIG.headers.cch_signing).toBe(false);
  });
});
...
describe("loadConfig", () => {
  ...
  it("merges headers sub-config", () => {
    ...
    const config = loadConfig();
    expect(config.headers.emulation_profile).toBe("claude-cli-2.1.90");
    expect(config.headers.overrides).toEqual({ "x-app": "custom-app" });
    expect(config.headers.disable).toEqual(["x-stainless-timeout"]);
  });

  it("falls back to the default profile when config references a removed profile", () => {
    existsSync.mockReturnValue(true);
    readFileSync.mockReturnValue(
      JSON.stringify({
        headers: {
          emulation_profile: "claude-cli-2.1.80",
        },
      }),
    );
    const config = loadConfig();
    expect(config.headers.emulation_profile).toBe("claude-cli-2.1.97");
  });
  ...
  it("accepts billing_header boolean from config", () => {
    existsSync.mockReturnValue(true);
    readFileSync.mockReturnValue(JSON.stringify({ headers: { billing_header: false } }));
    const config = loadConfig();
  ...
  it("accepts cch_signing boolean from config", () => {
    existsSync.mockReturnValue(true);
    readFileSync.mockReturnValue(JSON.stringify({ headers: { cch_signing: false } }));
    const config = loadConfig();
    expect(config.headers.cch_signing).toBe(false);
  });

  it("ignores non-boolean cch_signing values", () => {
    existsSync.mockReturnValue(true);
    readFileSync.mockReturnValue(JSON.stringify({ headers: { cch_signing: "no" } }));
    const config = loadConfig();
    expect(config.headers.cch_signing).toBe(false);
  });

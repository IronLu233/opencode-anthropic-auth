...
describe("refresh-lock", () => {
  ...
  it("does not release when inode changed even if owner matches", async () => {
    ...
    // Replace lock file with a different inode that reuses owner text.
    const originalStat = await fs.stat(first.lockPath);
    let replacementStat = originalStat;
    for (let i = 0; i < 5 && replacementStat.ino === originalStat.ino; i++) {
      await fs.unlink(first.lockPath);
      await fs.writeFile(first.lockPath, JSON.stringify({ owner: first.owner, createdAt: Date.now() + i }), {
        encoding: "utf-8",
        mode: 0o600,
      });
      replacementStat = await fs.stat(first.lockPath);
    }
